...
def main():
    ...
    clusterer = GaussianMixture(
        n_components=run.config.n_components_gauss,
        covariance_type=run.config.covariance_type,
        init_params=run.config.init_params,
    )
...
def sweep():
    ...
    parser.add_argument('--sweep_count', dest='sweep_count', type=int, default=150)
    ...
    sweep_config = {
        ...
        'parameters': {
            ...
            'init_params': {
                'values': ['kmeans', 'k-means++', 'random', 'random_from_data']
            },
    ...
    sweep_id = wandb.sweep(sweep_config, project="IDA-HW6-cluster-gaussian-mixtures") 
